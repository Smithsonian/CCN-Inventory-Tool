...
area_ha_mangrove <- signif(mangrove_area$area_ha, 4)
area_low_mangrove <- signif(mangrove_area$area_ha_lowerCI, 4)
area_high_mangrove <- signif(mangrove_area$area_ha_upperCI, 4)

#marsh values
marsh_area <- allstocks %>% filter(habitat == "marsh")
area_ha_marsh <- signif(marsh_area$area_ha, 4)
area_low_marsh <- signif(marsh_area$area_ha_lowerCI, 4)
area_high_marsh <- signif(marsh_area$area_ha_upperCI, 4)


#seagrass values 
seagrass_area <- allstocks %>% filter(habitat == "seagrass")
area_ha_seagrass <- signif(seagrass_area$area_ha, 4)
#no CIs
```
...
stock_country <- signif(countrystock$Total_Stocks, 4)
stock_country_low <- signif(countrystock$Total_Stockers_LowerCI, 4)
stock_country_high <- signif(countrystock$Total_Stockers_UpperCI, 4)
stock_country_se <- signif(countrystock$Total_SE, 4)
...
#mangrove
totalstocks_mangrove <- signif(mangrove_stocks$Total_Stocks, 4)
totalstocks_mangrove_upper <- signif(mangrove_stocks$area_ha_upperCI, 4)
totalstocks_mangrove_lower <- signif(mangrove_stocks$area_ha_lowerCI, 4)
  
#marsh
totalstocks_marsh <- signif(marsh_stocks$Total_Stocks, 4)
totalstocks_marsh_upper <- signif(marsh_stocks$area_ha_upperCI, 4)
totalstocks_marsh_lower <- signif(marsh_stocks$Total_Stocks_LowerCI, 4)


#seagrass 
totalstocks_seagrass <- signif(seagrass_stocks$Total_Stocks, 4)
totalstocks_seagrass_upper <- signif(seagrass_stocks$area_ha_upperCI, 4)
totalstocks_seagrass_lower <- signif(seagrass_stocks$Total_Stocks_LowerCI, 4)
...
We estimate that **`r chosen_geog`** contains a mean estimate of `r stock_country` megagrams per hectare sediment carbon stock. 

We estimate that **`r chosen_geog`** contains between `r stock_country_low` to `r stock_country_high` megagrams per hectare sediment carbon to a depth of 1 m, with a mean estimate of `r stock_country` megagrams per hectare.
...
totalstocks_mangrove <- signif(totalstocks_mangrove, 4)
...
This total estimate also includes total tidal marsh carbon stocks, with a mean estimate of `r totalstocks_marsh` megagrams per hectare soil carbon to a depth of 1 meter.
...
This estimate also includes seagrass soil carbon stocks for `r chosen_geog` which we estimate to have a mean of `r totalstocks_seagrass` megagrams per hectare sediment carbon stock.
...
Within the Coastal Carbon Atlas, data from the selected geography, `r chosen_geog`, includes soil samples and vegetation samples.

Soil core data from **`r chosen_geog`** included in the Coastal Carbon Atlas consists of `r cores` soil profiles from `r sites` sites. This data comes from `r habitats` distinct habitat type(s). At present, there are `r veg_samples` vegetation samples. 
...
allstocks %>% 
  ...
  facet_wrap(~`carbon_pool`, 
             # scales = "free", 
             dir = "v")+
  labs(title = "Emissions Factors: Palau")
